...
class HealthCalculator:
    ...
    def _get_priced_account_instrument(
        self, asset, open_orders_by_address, group, cache
    ):
    ...
    def calculate_assets_liabs(
        ...
        priced_reports = [
            (
                asset,
                self._get_priced_account_instrument(
                    asset, open_orders_by_address, group, cache
                ),
            )
            for asset in account.base_slots
        ]
        assets_spot, liabs_spot = map(
            sum,
            zip(
                *[
                    (report.deposit.value, report.borrow.value)
                    for _, report in priced_reports
                ]
            ),
        )
        assets_perp, liabs_perp = zip(
            *[
                (
                    asset.perp_account.asset_value(
                        report.perp_market_cache, report.price.value
                    ),
                    asset.perp_account.liability_value(
                        report.perp_market_cache, report.price.value
                    ),
                )
                for asset, report in priced_reports
            ]
        )
        ...
        liabs_perp = sum(
            [perp for perp in liabs_perp]
        )  # somehow the USDC pricing is off
    ...
    def calculate(
        ...
        for priced_report in priced_reports:
            ...
            health += (
                priced_report.raw_perp_quote_position
            )  # not sure why this is added, causes the numbers to be way off
